...
- **🏁 MILESTONE — Showcase truth rebuild: audit → 4 phases, arc CLOSED (2026-07-15).** The showcase can no longer silently lie: roster derives from the package barrels at build time (**113→185 browsable, misc=0**; tier/function via `showcase/src/lib/classification.js`); **three CI gates** (`validate:roster`/`:imports`/`:foundations`) + JSDoc-description extraction (136/185, build-fresh) wired into `pnpm build` + release.yml; vendored `showcase/src/workshop/` fork **deleted** (imports = `@kolkrabbi/*` only, gate-enforced); usage miner re-rooted (`kol-apps`+`kol-website`, hard-fail) and re-mined live; install commands attribute the true owner; **⌘K search works** (ShellSearchOverlay over the roster); sets/blocks carry **provenance badges**; `/lobby`+`/demo` dev-only. Same day: `/work` backport seams → **content 0.2.0 (npm ✓)**; **framework 0.4.0 (npm ✓)** with the dark-imposition fix. **No open threads** — residuals parked at `backlog/2026-07-15-parked-threads.md`. Capstone: `session-log/2026-07-15-MILESTONE-showcase-truth-rebuild.md` · journal: `playbook/2026-07-15-showcase-truth-rebuild.md`.

- **✅ Raid batch published + DS dark-imposition bug killed (2026-07-15).** `pnpm install` relinked the moved repo (blocker cleared); missing `kol-styleguide` dep added to showcase (raid wired demos, never declared it — build failed); **6-package batch published + verified 15/15 local==npm** (styleguide 0.1.0 new · component 0.9.0 · foundry 0.4.0 · brand-template 0.2.0 · theme 0.7.4 · framework 0.3.5). Then three fixes: ThemeToggle icons → v1 `mode-toggle-01/02` (legacy warning gone); TopBar search `sm:block`→`sm:inline-flex` (stacked row); **ThemeToggle no longer stamps `data-theme='dark'` on mount** (since framework 0.1.0, never a decision — policy is light-first / follow system / dark = per-repo). Init: attribute → saved toggle → `prefers-color-scheme`; writes only on user toggle. **framework 0.4.0 CI-verified on npm** (closed at the 🏁 milestone). Lobby real queue = 4 (ParaType · InteractiveImage · TailwindContentSource · TypeScaleSection); the INDEX ghost-row rewrite is **parked** at `backlog/2026-07-15-parked-threads.md`. Detail: `session-log/2026-07-15-batch-publish-themetoggle-theme-default-fix.md`.
...
- *(Older 07-09 entries — lobby agent-fanout processing, lobby wall + publish chain, workshop type-conformance, the foundry 0.2.0 cut, wordmark wiring — trimmed per the 5-entry cap; see `session-log/2026-07-09-lobby-processing-agent-fanout.md`, `session-log/2026-07-09-lobby-wall-publish-chain-vault-docs.md`, see `session-log/2026-07-09-foundry-cut-rename-store-extract-workshop-type-conform.md` and `session-log/2026-07-09-workshop-wordmark-wiring-content-doc.md`.)*
