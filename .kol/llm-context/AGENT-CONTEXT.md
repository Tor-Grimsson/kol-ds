...
- **✅ Chess 0.4.1 + theme 0.9.1 — palette pawns + amber sidelines (2026-07-15, night, closing).** User live-testing brief 3.0 caught the mock-era palette (rook…rook mirror, NO pawn → removed pawns unrecoverable) → palette is now the six real piece types; armed cell = `.chess-palette--armed`; sideline notation moves = `.kol-btn.chess-notation-sideline` (theme-aware amber via color-mix). Published + consumer-verified same night. **▶ NEXT SESSION: the website leg** (user clearing context for it) — to-dos in `playbook/2026-07-15-two-repo-handoff.md`, noting theme is now **0.9.1** not the 0.8.0 the handoff says. Detail: `session-log/2026-07-15-chess-0-4-1-palette-amber.md`.

- **✅ Brief 3.0 (chess board interactivity) executed + PUBLISHED (2026-07-15, late night).** The chess package got its input layer: `ChessBoard` `interactive`+`onMove` click-to-move (legal targets via chess.js, theme classes `--selected/--target/--target-capture`), a **flat sidelines model** in the controls context (off-mainline moves branch; inline `(1.Na3 e5)` rows in NotationPanel; Copy-PGN emits real `(...)` variations; cursor-aware stepping/playback), edit-mode palette placement, `window.prompt` flow deleted. **Context API rename:** `userVariations`-family → `sidelines`/`playMove`/`goToSidelineMove`/`removeSideline`/`getPgnWithVariations` (+`activeFen`/`selectPly`). Acceptance ×5 verified live in showcase; gates green. **Published via user push: chess 0.4.0 · theme 0.9.0** (landed ~20s); consumer bumped + verified same night (its own context has the log). Detail: `session-log/2026-07-15-brief-3-board-input-sidelines.md`.
...
- **✅ Ledger 2.0 closed 6/6 + 🚩 sidenav epic flagged (2026-07-15, late night).** DS-CHANGES-2.0 looped under /kol-goal: `titleClassName` shipped (content 0.4.0) · `tagsSeparator` recorded as already-delivered · ThemeToggle icon variant → user-verified 36/20 (framework 0.5.0) · auto-dark block RULED KEPT (light-first + follow-system policy, documented at the block) · kol-foundry ruled canonical (deprecate command handed to user) · sidebar rhythm ruled final after user review (rail keeps `kol-helper-10`, 6px rows unified, untyped wrappers typed — theme 0.8.0 + workshop **0.1.6**). Outcomes table in `_tmp/DS-CHANGES-2.0.md`. **🚩 User flagged the sidenav family as wrong/incomplete — next epic, HIS scoping doc pending; surface captured at `backlog/2026-07-15-sidenav-epic.md` — don't redesign from the note alone.** **⏳ Batch cleared for push (push == publish): component 0.11.0 · chess 0.3.0 · framework 0.5.0 · content 0.4.0 · theme 0.8.0 · icons 0.7.0 · foundry 0.4.2 · workshop 0.1.6.** Detail: `session-log/2026-07-15-ledger-2.0-loop-sidenav-flag.md`.
...
- **Live npm versions (2026-07-15 night, verified on the registry):** chess **0.4.1** · theme **0.9.1** (brief-3.0 + follow-up pushes) on top of the shipped evening batch — component **0.11.0** · framework **0.5.0** · content **0.4.0** · icons **0.7.0** · foundry **0.4.2** · workshop **0.1.6**. Older tier: brand **0.1.2** · styleguide **0.1.0** · brand-template **0.2.0** · store **0.1.1** · dashboards/media-client/scrape **0.1.0**. Canonical table: `docs/operations/SHIPPED-PACKAGES.md`. Publish mode is still **push == publish** (zero changesets); local `pnpm publish --no-git-checks` also works OTP-free (granular token in `~/.npmrc`). **Recurring trap:** editing package source without bumping = same-version/different-content vs npm — always bump with the edit. New-package note: first publish can lag the registry read-path a few minutes (styleguide took ~4). `kol-loader@0.3.0` orphaned on npm — deprecate → kol-icons when ready.
