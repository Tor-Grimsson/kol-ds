...
- **🏁 MILESTONE: the shell-adoption night — ONE chrome, ONE pipeline (2026-07-30, latest) — ARC CLOSED.** The showcase stopped hand-maintaining its own chrome and its own docs format. `ShellChrome.jsx` mounts the packaged workshop shell ONCE as a route-level layout; `DocLayout`/`TopBar`/`NavDrawer`/`SidebarNav` are **deleted**; every page is content only; the TOC is derived from rendered headings (all hand-written arrays gone); `@mdx-js/rollup` is wired so pages are documents with `<Preview>`/`<Api>` embedded. The kol-website fork (2,749 vendored lines) is folded back into the package and can drop its alias. Five shell defects fixed that both copies carried — dead mobile drawer, dead ⌘K, rail desync, doc trees listing only INDEX files, reader crashing outside a shell. Capstone: `session-log/2026-07-30-MILESTONE-shell-adoption-mdx-pipeline.md`. **Nothing open in this repo** — remaining MDX prose is parked at `backlog/2026-07-30-mdx-content-migration.md`; device testing at `backlog/2026-07-29-device-testing-audit.md`; the sidenav epic is SUPERSEDED.
- **⚖️ Standing laws set this arc.** (1) **Links ship colorless** — `--kol-link` is `currentColor`, a per-repo hook; the DS ships no link color. (2) **Theme is tri-state** — explicit > system > light, with `system` a real state that clears the stamp. (3) **Cascade LAYER is contract, not just order** — framework CSS imports with `layer(components)`; unlayered rules outrank every layered one, which made one package render differently in two apps. (4) **A component's type lives in its own rule**, never a `kol-mono-*` utility racing it. All four written into ARCHITECTURE §5 / READMEs / topology doc.
- **📦 Published this arc:** theme **0.12.1** · component **0.13.1** · framework **0.6.4** · icons **0.8.10** · workshop **0.3.0**. Gates 5/5, production build green. Canonical table: `docs/operations/SHIPPED-PACKAGES.md`.
- **✅ Lobby audit cleared** — all 9 tasks of `lobby/DocTableAndChipAudit.md` closed (doc-table cell roles split on the mono fault line, Pill/Tag chip-law defaults, duplicate class homes deleted, `.kol-fs-tile` ruled dead).
- **🧭 New docs surface:** `docs/documentation/08-breakpoints/` — the breakpoint lookup (values · best practices · testing methods · KOL-DS rules).
