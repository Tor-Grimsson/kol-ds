...
- **✅ AUTO-DARK KILLED — theme 0.9.2, USER OVERRULE of ledger 2.4 (2026-07-15, late night, latest).** The ledger-2.0 loop's "KEEP auto-dark" closure was reversed: that item was *held for user* and should never have been agent-closed. **Standing ruling: light mode first until migration completion** — the `@media (prefers-color-scheme: dark)` block + its false policy comment deleted from `kol-color.css`; tokens doc synced; gates 5/5; published via user push, consumed + verified by kol-website same night. Also closed: kol-specimen npm-deprecated (user ran it, flag verified). Website ledger carry: 2.8/2.9/2.10. **Lesson: held-for-user = never agent-closable.** Detail: `session-log/2026-07-15-autodark-kill-theme-0-9-2.md`.
