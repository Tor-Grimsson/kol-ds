...
- **✅ Chess board height fence — chess 0.5.1 + theme 0.11.2 (2026-07-28, latest).** Consumer-reported (kol-chess app): board stuck small on wide desktops — theme's `.chess-board--fluid` hard-capped at 760px, and the lg stage was width-driven (`lg:max-w-none`, the dvh caps were mobile-only). Theme: cap dropped, height fence reads `--chess-stage-reserve` (default 200px). Chess: stage self-caps + centers off 100dvh at lg (panel budgets −106px); showcase mirrors (Demo + chess-apparatus set) drop the 1232px shell cap 1:1. Verified /demo 1920×1200 (board 760→1000, rail flush, no scroll). Published, SHIPPED-PACKAGES bumped, **git push = user's (pinged)**; consumer bump lands app-side same session. Log: `session-log/2026-07-28-chess-board-height-fence-0-5-1-theme-0-11-2.md`.
- **✅ Workshop shell scroll regions — kol-workshop 0.1.7 (2026-07-28).** Consumer-reported (kol-website): one scroll container wrapped the whole grid — rails co-scrolled on a sticky+max-h hack, cut at the bottom, bounce chained. `ShellLayout` rebuilt: Nav/Toc/Main each own `overflow-y-auto` regions (`min-h-0`), main locks in full-height mode (embeds), `overscroll-none` everywhere. No API change. Published, SHIPPED-PACKAGES bumped, **git push = user's**. Log: `session-log/2026-07-28-workshop-shell-scroll-regions.md`.
- **🏁 MILESTONE: the website leg — four theme releases + three lobbies (2026-07-16).** The DS side of the two-repo mandate is CLOSED: theme **0.9.2** (auto-dark deleted — **standing USER law: light-first until migration completion**; an agent-closed held-for-user ruling was overruled, lesson codified) → **0.9.3** (`.kol-prose` on the kol-opacity text roles) → **0.10.0** (**Display Tight tier** — `kol-display-lg/-section/-section-sm/-subsection`, lobbied verbatim, own token ladder) → **0.11.0** (fontviewer chrome; `--fv-*` conformance flagged in `packages/foundry/COMPONENTS.md`) · **component 0.12.0** (FoundryCTA, the centered tier of the two-variant CTA family) · **foundry 0.5.0** (fontviewer engine + GlyphItem at `./engine`). All user-pushed, consumer-verified; gates 5/5, roster 216; kol-specimen npm-deprecated (flag live). apps/web is zero-elder outside its workshop joint scope. Website ledger carry parked in `_tmp/DS-CHANGES-2.0.md`: 2.8/2.9/2.10. Capstone: `session-log/2026-07-16-MILESTONE-website-leg-publish-waves.md`.
...
- **Live npm versions (2026-07-28, verified on the registry):** theme **0.11.2** (fluid board 760px cap dropped, `--chess-stage-reserve` fence) · component **0.12.0** · foundry **0.5.0** · chess **0.5.1** (stage self-caps + centers off 100dvh at lg) · framework **0.5.0** · content **0.4.0** · icons **0.7.0** · workshop **0.1.7** (2026-07-28 shell scroll regions) · dashboards **0.2.0** (kol deps → peerDeps; first real consumer = kol-chess `/stats`). Older tier: brand **0.1.2** · styleguide **0.1.0** · brand-template **0.2.0** · store **0.1.1** · dashboards/media-client/scrape **0.1.0**. Canonical table: `docs/operations/SHIPPED-PACKAGES.md`. Publish mode is still **push == publish** (zero changesets); local `pnpm publish --no-git-checks` also works OTP-free (granular token in `~/.npmrc`). **Recurring trap:** editing package source without bumping = same-version/different-content vs npm — always bump with the edit. New-package note: first publish can lag the registry read-path a few minutes (styleguide took ~4). `kol-loader@0.3.0` orphaned on npm — deprecate → kol-icons when ready.
