...
## Current state (2026-09-22, kol-olina's five media tickets)

- **🎫 All five DS blockers kol-olina filed against media.olina-productions.com fixed and proved live in `apps/media` (2026-09-22, newest): component 0.216.0 → 0.217.0 + theme 0.147.0 (published 2026-09-23).** `useModal()` gained `alert`; every `prompt()`/`confirm()`/`alert()` in `MediaLibraryPages.jsx` now goes through it. `fileActions.items` — consumer-defined menu entries rendered after the built-ins, same busy/error path — proved with a fixture **Duplicate**. `onDropFiles(files, folderPath)` on `ColumnBrowser` + both media pages: OS files dropped on a folder or column get the same highlight as an internal drag and the DS never uploads. `ColumnBrowser`'s `height` takes any CSS length now, not just px — the resize drag measures the rendered rect as its base so a string height doesn't jump — and `apps/media` uses `calc(100dvh - 224px)` to fill the viewport. Native `title` swept for **the media pages' own controls** (`ViewToggle`, `SettingsPanel`, `ContentFilters`' layout strip, `ActionButton`) → the DS `Tooltip`, with a new nesting guard so a disabled-switch hint doesn't fight its row's hint. ⚠️ **Scope was media only, on purpose** — the editor-chrome components (`LayerStack`, `TimelineDock`, `SelectionOverlay`, `ColorInputRow`, `ShapeDropdown`, `ColorRamp`, `SpectrumGrid`) still carry native `title`; no ticket asks for them and I wrongly told the user one did before checking. ⚠️ **Found, not fixed**: a pre-existing bug in Column view — dropping a file onto another file's row renames the dragged file to the target's name instead of refusing. Log: `session-log/2026-09-22-olina-media-tickets.md`.

## Current state (2026-09-22, MBP setup)

- **💻 The repo made workable on the MBP (2026-09-22).** Fresh clone: `pnpm install`, 26 gates clean, project memory copied from the iMac, `npm login` as `tor-grimsson`. **Changesets CI retired** — `release.yml` → `_tmp/2026-09-22-mbp-setup/`; its last act was a stale "Version Packages" PR #9 (closed, branch deleted). Its three gates still run in `pnpm build`. ⚠️ **The estate addresses this repo as `~/dev/projects/kol-ds-ui`** (lobby-ds, `kol-link`, `repo-map`, every outbox receipt) — the MBP clone came down as `kol-ds` and must be renamed, memory folder with it. ⚠️ iMac user is `biskup`, MBP `kolkrabbi`; the usage/token miners hardcode `/Users/biskup` and run on the iMac only. Next: kol-olina's DS-blocker tickets. Log: `session-log/2026-09-22-mbp-setup.md`.
