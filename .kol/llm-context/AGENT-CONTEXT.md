...
- **✅ Workshop shell scroll regions — kol-workshop 0.1.7 (2026-07-28, latest).** Consumer-reported (kol-website): one scroll container wrapped the whole grid — rails co-scrolled on a sticky+max-h hack, cut at the bottom, bounce chained. `ShellLayout` rebuilt: Nav/Toc/Main each own `overflow-y-auto` regions (`min-h-0`), main locks in full-height mode (embeds), `overscroll-none` everywhere. No API change. Published, SHIPPED-PACKAGES bumped, **git push = user's**. Log: `session-log/2026-07-28-workshop-shell-scroll-regions.md`.
