...
| **3. Attribution + prose ✅ EXECUTED 2026-07-15** | pkg attribution landed with ph1 enumerator · `extract-descriptions.mjs` (JSDoc first-sentence, 136/185, build-regenerated; DESCRIPTIONS demoted to fallback) · `resolveTokenThemed` CSSOM walk + `measureClass` probe → every Foundations cell live-read, literals deleted · `validate:foundations` gate in build + release.yml | P0-4, P1-4, P1-5 (P2-2/P2-4 = phase-4 hygiene) |
| **4. Provenance + search ✅ EXECUTED 2026-07-15** | composition extractor derives from packages/ via parse-barrel (P2-3) · ProvenanceBadge on every /blocks + /sets card + stage header · TopBar search = SearchInput trigger + ShellSearchOverlay + ⌘K over the roster · /lobby + /demo + Lobby nav link behind `import.meta.env.DEV` | P1-1, P1-2, P1-3 (nav half), P2-3 |

**ARC COMPLETE 2026-07-15** — all four phases executed; every P0 and P1 finding dead. Residual accepted: P2-2 (dead fallback keys — harmless since descriptions extract from JSDoc), P2-4 (Home hero copy understates the surface — user-facing text, owner's call). This doc → the playbook has the execution journal; invariants promoted into `docs/documentation/03-components/00-taxonomy.md`.
