...
🟢 `closed`. **Shipped and published:** `@kolkrabbi/kol-component@0.20.0` +
`@kolkrabbi/kol-theme@0.18.1` (registry, 2026-08-01). 14/14 gates clean,
production build green.

**The brief asked for a ternary swap. It got the cause instead** — its own
Recreation notes named it: *"Consider hoisting both ladders to one shared module…
That is the real fix; the ternary swap is the patch."*

| | What landed | Where |
|---|---|---|
| new | `SOLO` 16/20/24 · `ADJACENT` 14/16/18 · `glyphSize(size, solo)` | `packages/component/src/hooks/glyphLadders.js` |
| fix | `iconSize ?? glyphSize(size, Boolean(iconOnly))` — the defect | `atoms/Button.jsx:65` |
| dedupe | local `GLYPH` const → the shared `SOLO` | `atoms/IconFrame.jsx:54` |
| new prop | `radius` `sm`\|`full`, mirroring IconFrame | `atoms/Button.jsx` |
| CSS | `.kol-icon-frame-radius-full, .kol-btn-radius-full` — **one rule, two selectors** | `theme/kol-components-atoms.css:896` |
| docs | new **Glyph ladders** + **Radius** sections | `docs/documentation/03-components/05-control-chrome.md` |

**Not a new `lib/` folder.** `src/hooks/` is stated in-repo as *"the taxonomy's
only non-component folder"* (`hooks/cssVar.js:5`), and it already holds two
non-hook modules (`colorMath.js`, `cssVar.js`). The ladders went there.

**No new radius token.** The brief's claim was re-checked and holds: no `--kol-*`
carries a full round. Rather than transcribe the literal a second time, the two
classes share one rule — so a future token has exactly one selector to land in.

**Visual break, as filed:** every bare `iconOnly` Button gains 2px of glyph. Both
in-repo call sites (`ShapeDropdown.jsx:69`, `AlignmentGrid.jsx:43`) pass explicit
`iconSize` and are unaffected. Minor bump, not a patch.

### 🔴 Left open — the brief's own question, sharpened

`Input.jsx:34` sets `ICON_SIZE = { sm: 14, md: 14, lg: 18 }`. Its **`md` is 14
where ADJACENT says 16**, in the same text-adjacent role. That is a *third*
ladder nobody declared — the brief noted four transcriptions but not that they
disagree. Not decided here: either it is a named control-chrome rung or it is
drift, and that is the user's call. `Tag`'s `10/12/14` is chip scale and left
alone deliberately.

### ✅ …and closed — 2026-08-01, same day

`Input.jsx` **was drift, not a rung.** Ruled by the user, shipped in
`@kolkrabbi/kol-component@0.20.1`.

The evidence that settled it: `.kol-control-sm/md/lg` and `.kol-btn-sm/md/lg`
carry **identical padding and type** per rung (`4px 12px`/mono-12 ·
`6px 16px`/mono-14 · `8px 20px`/mono-16), so an Input's icon sits in the same
line box as a labelled Button's. Input's own `sm: 14` and `lg: 18` had always
matched ADJACENT — only `md` diverged, and nothing commented on it. A deliberate
quieter-control rung would have lowered all three.

Local `ICON_SIZE` deleted; `Input` reads `glyphSize(size)`. Two in-repo call
sites move (`chess/SetupPanel.jsx:72`, `chess/GameArchiveTable.jsx:438` — both
default `md`, both gain 2px). No API change, so a patch.

**Four transcriptions are now one**, plus `Tag`'s `10/12/14` kept on purpose:
chip scale is a smaller family, not a third opinion about the control rungs.
