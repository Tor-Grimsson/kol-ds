...
- **Every file gets a numeric or date prefix.** Exception: `INDEX.md` is exempt.
  ...
  - **Generated folders** (machine-written catalogs, e.g. a mined per-component usage reference): files keep their generator's naming, unprefixed — the generator owns the folder; don't hand-rename its output. Mark the folder as generated in its parent INDEX.
