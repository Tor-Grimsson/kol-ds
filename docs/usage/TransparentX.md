# TransparentX

- **Package:** `@kolkrabbi/kol-component`
- **Category:** atoms
- **Real-world usages found:** 9 across 8 files in 7 apps
- **Used in:** kol-client-ac, kol-client-hrafn, kol-client-kolkrabbi, kol-editor, kol-labs-single, kol-lightroom, kol-media-admin

## Import

```jsx
import { TransparentX } from '@kolkrabbi/kol-component'
```

## Real usage

From `kol-apparat/kol-plugin/kol-media-admin/src/components/atoms/ColorSwatch.jsx`:

```jsx
<TransparentX tone={transparentTone} />
```

From `kol-apparat/kol-editors/kol-editor/docs/editor-port/from-kol-ac/color-review-refs/generators/social/ColorPicker.jsx`:

```jsx
<TransparentX />
```
