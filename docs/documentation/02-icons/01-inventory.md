---
title: Icon inventory — kol-icon-set-v1 names by group
type: reference
status: active
updated: 2026-07-28
description: The full per-group roster of kol-icon-set-v1 names — THE packaged set since 0.8.0 (legacy stroke/solid trees removed; local shelf at _tmp/legacy-icons). Generated from packages/icons/src/kol-icon-set-v1 — regenerate after any cull or promotion.
aliases:
  - icon inventory
  - icon list
sources:
  - packages/icons/src/kol-icon-set-v1
tags:
  - domain/design-system
  - domain/iconography
related:
  - "[[INDEX|icons]]"
---

# Icon inventory — kol-icon-set-v1 names by group

Generated from `packages/icons/src/kol-icon-set-v1/` — **regenerate after any cull or promotion** (not auto-synced; the live `/icons` page and the keys-only `KOL_ICON_SET_V1` export are the runtime source of truth). See [[INDEX|icons]] for the loader model.

**143 icons · 22 groups** (2026-07-28, post v1-only cutover — 17 promotions landed with 0.8.0). Legacy stroke/solid rosters live only on the local shelf `_tmp/legacy-icons/` and in published `kol-icons@0.7.1`.

| Group | Count | Names |
|---|--:|---|
| `add-remove` | 4 | `check` · `minus` · `plus` · `x` |
| `arrow` | 6 | `arrow-down` · `arrow-downright` · `arrow-left` · `arrow-right` · `arrow-up` · `arrow-upright` |
| `atomic` | 10 | `atomic-atom` · `atomic-atomic-01` · `atomic-atomic-02` · `atomic-molecule-01` · `atomic-molecule-02` · `atomic-molecule-03` · `atomic-organism-01` · `atomic-organism-02` · `atomic-organism-03` · `atomic-organism-04` |
| `caret` | 4 | `caret-down` · `caret-left` · `caret-right` · `caret-up` |
| `chevron` | 5 | `chevron-down` · `chevron-expanded` · `chevron-left` · `chevron-right` · `chevron-up` |
| `code` | 6 | `bug-beetle` · `code` · `code-simple` · `hash-01` · `hash-02` · `terminal` |
| `components` | 5 | `component-01` · `component-02` · `instance-01` · `instance-02` · `variant-01` |
| `device` | 3 | `desktop` · `mobile` · `tablet` |
| `eye-lock` | 5 | `eye-off` · `eye-on` · `lock` · `shield-check` · `unlock` |
| `files` | 10 | `book-open` · `clipboard` · `database` · `file` · `folder` · `image` · `journal` · `library` · `roadmap` · `video` |
| `layout` | 8 | `grid` · `layers` · `layout` · `maximize` · `panel-left` · `panel-right` · `row` · `view-list` |
| `misc` | 8 | `chess-pawn` · `chess-rook` · `foundation` · `resize-grip` · `scribble` · `stat-chart-a` · `trending-down` · `trending-up` |
| `nav` | 6 | `hamburger` · `home-01` · `home-02` · `more` · `settings-01` · `user` |
| `notify` | 6 | `attach` · `bell` · `email-01` · `email-02` · `fingerprint` · `message` |
| `playback` | 6 | `pause` · `play` · `skip-back` · `skip-end` · `skip-forward` · `skip-start` |
| `shape-forms` | 7 | `bolt` · `bookmark` · `bucket` · `cloud` · `drop` · `flag` · `heart` |
| `shape-primitives` | 9 | `circle` · `cone` · `diamond` · `polygon` · `rectangle` · `square` · `star` · `star-solid` · `triangle` |
| `singletons` | 10 | `alert-triangle` · `copy` · `cut` · `edit` · `external-link` · `filter` · `info` · `save` · `search` · `trash` |
| `social` | 1 | `social-github` |
| `tools` | 18 | `align-horizontal-center` · `align-horizontal-left` · `align-horizontal-right` · `align-vertical-bottom` · `align-vertical-center` · `align-vertical-top` · `brightness` · `customize` · `dashed-circle` · `eyedrop` · `frequency` · `mode-toggle-01` · `mode-toggle-02` · `overlap` · `pointer` · `swap` · `target` · `type` |
| `traffic` | 4 | `direction-cross` · `direction-opposite` · `refresh` · `repeat` |
| `transfer` | 2 | `download` · `upload` |
