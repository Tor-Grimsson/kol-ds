...
| **SegmentedToggle** | molecule | Chrome + full size alignment to Button (sm/md/lg pixel-matched), sm/md rescale, `radiogroup` a11y. | Padding-driven sizing that pixel-matches the button scale (22/26/32/40); `radiogroup`/`radio` + arrow keys + focus-visible. | `segmented-toggle-button-sizes`, `segmented-toggle-chrome` · 8 logs |
