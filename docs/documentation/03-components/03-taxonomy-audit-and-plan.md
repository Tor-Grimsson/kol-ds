...
| **Loaders** | Name→asset resolvers (Icon, Graphic). Not visual UI. | `kol-icons`, `graphics/` |
...
- New definitions-first doc (`03-components/00-taxonomy.md` or fold into a renamed placement doc) that owns: the two axes, the five tier definitions **+ the explicit Brad-Frost divergence statement**, the Function set, and the "one page per component" rule.
