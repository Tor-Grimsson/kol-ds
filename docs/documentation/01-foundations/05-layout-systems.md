...
| **Padding ladder** (`--kol-pad-*`) | page/section/band padding rhythm for **page content** — and since theme 0.135.0 the shell page's too: `--kol-shell-page-pad` is an alias of `--kol-pad-section-x` (two-page-scaffolds-one-job, 2026-09-03 — the shell had a second ladder that agreed with this one only at 48) | 3-breakpoint responsive ladder (20 · 32 · 48) | `kol-framework.css` `:root` blocks · the shell alias in `kol-components-shell.css` | shell chrome — that is the chrome inset below; a second gutter ladder anywhere |
