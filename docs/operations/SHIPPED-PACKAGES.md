...
> Versions below = the **staged 2026-07-15 batch** (push == publish). Verify against npm after CI runs.

## UI tier

| Package | Version | Job |
|---|---|---|
| `@kolkrabbi/kol-theme` | **0.8.0** | Foundation CSS — tokens, type classes, all component chrome (`.kol-btn*`, `.kol-prose`, per-domain sheets) |
| `@kolkrabbi/kol-icons` | **0.7.0** | `<Icon>` + the curated `kol-icon-set-v1` + `registerIcons` (BYO) |
| `@kolkrabbi/kol-component` | **0.11.0** | The components — atoms → molecules → organisms + `<Graphic>` |
...
| `@kolkrabbi/kol-chess` | **0.3.0** | Chess apparatus — board, 3 piece sets, playback/notation/variations, `./data` adapter |
