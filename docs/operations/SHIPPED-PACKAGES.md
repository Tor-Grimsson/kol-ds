...
| Package | Version | Job |
|---|---|---|
| `@kolkrabbi/kol-theme` | **0.11.25** | Foundation CSS — tokens, type classes, all component chrome; 0.11.9/0.11.10 the `kol-doc-*` (11) + `kol-card-*` (6) type-role sets (footer mono 10/14; 0.11.11 radius law 4px; 0.11.14-19 CodeBlock chrome verbatim from the live reference + doc-body tracking 0.04em + table scroll edge = surface fade only (no shadow); 0.11.20-22 `--kol-content-*` = ONE frame + two caps (shell 1800 · column 768 · measure 65ch; page/wide tiers killed same day — width is content, not page identity)) (`kol-type-roles.css`, prose-twinned furniture, opt-in); 0.11.7 nav opacity ladder; 0.11.25 kol-color.css dark-mode header cites the corrected law (explicit > system > light) | 
| `@kolkrabbi/kol-icons` | **0.8.8** | `<Icon>` + kol-icon-set-v1 (166 icons / 26 groups per the 0.8.7 inventory regen; diagonals in their own arrow-diagonal group; 0.8.4 v1 `copy` = the elder glyph, old one retired to the shelf) + `registerIcons` (BYO); 0.8.0 v1-ONLY — legacy stroke/solid/svg/svg-web trees removed (local shelf `_tmp/legacy-icons/`), `variant` prop dropped, 17 promotions in; 0.8.1 keyline conformance — chevrons+carets fit 18×18, expanded redrawn stroke-1.5, arrow family complete: 4 diagonals baked from the downright master; chevron-expanded killed (twin of chevron-up) |
| `@kolkrabbi/kol-component` | **0.12.19** | The components — atoms → molecules → organisms + `<Graphic>`; 0.12.5 CodeBlock REPLICATED from the elder reference (react-syntax-highlighter + oneDark, filename-or-language chip, 32×32 icon copy w/ `copy`→check); 0.12.6 Figure rides the doc-eyebrow/caption roles; 0.12.7 Dropdown auto-size = sm at every viewport unless explicit `size` (user law) + ContentFilters view-mode toggle → ViewToggle (active filled chip / inactive bare) |
| `@kolkrabbi/kol-framework` | **0.6.0** | App shell — `AppShell`, `SideNav`, `ShellHeader`, `ThemeToggle` + `useTheme`, heroes, footer; 0.5.5 icon names → v1 (`hamburger`/`x`); 0.5.4 theme state = explicit > system > light; 0.5.8 codeblock chrome moved OUT (home = kol-theme); **0.6.0 tri-state theme** — ThemeToggle cycles light → dark → system (`system` clears stamp + storage, follows the OS live; `desktop` glyph), `useTheme` adds `mode`/`cycle`, `applyTheme('system')` |
