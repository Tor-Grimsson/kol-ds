...
type: plan
status: superseded
superseded_by: "[[workbench/01-using-the-workbench|using the workbench]]"
updated: 2026-06-26
...
related:
  ...
  - "[[migration/2026-06-18-src-first-restructure|src-first restructure]]"
...
**Decoupled from the open src-first decision.** The workbench depends only on `@kolkrabbi/*` specifiers, which the [[migration/2026-06-18-src-first-restructure|src-first restructure]] preserves by design — so this survives that move (or its rejection) untouched. Safe to build before that call lands.
