...
// An apps/* member: runs locally on `pnpm notes` (the notes tool alone on the fixture, 2026-09-27), and publishes into the
// showcase's output so the current state is viewable at ui.kolkrabbi.io/apps/notes.
...
export default defineConfig(({ command }) => ({
  ...
  // on its own port, so `pnpm notes` stays an ordinary localhost root.
  base: command === 'build' ? '/apps/notes/' : '/',
  ...
  build: {
    ...
    outDir: '../../showcase/dist/apps/notes',
    emptyOutDir: true,
  },
