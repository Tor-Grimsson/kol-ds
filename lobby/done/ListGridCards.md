...
## ✅ CLOSED — 2026-08-30 · purpose served

The collection existed to hold evidence until a card-family consolidation was
ruled. **It was ruled and shipped today**, so there is nothing left to collect
for.

### The nine, resolved

| # | collection entry | outcome |
|---|---|---|
| 1 | `MediaCard` + `MediaRow` | → `ContentCard`/`ContentRow variant="file"` · exports dropped |
| 2 | `GridCard` (kol-shell) | → `catalog` · export dropped |
| 3 | `ListingCard` (+`ArticleCard`) | → `article` · exports dropped |
| 4 | `WorkCard` + `WorkListItem` | → `showcase` · exports dropped |
| 5 | `TypefaceLibraryItem` | → `showcase layout="canvas"` · export dropped |
| 6 | `PrintGridCard` | → `catalog` + `flip`/`fade` props · export dropped |
| 7 | `PrintGridCardGsap` | **still ships** — never in the wave |
| 8 | client-repo icon tiles / `ShopGrid` | consumer-local, no DS component; the six forked `ContentFilters` copies were named on `ContentSetRetirement` |
| 9 | `Table` | not a card, never was |

Shipped as component 0.132.0 · shell 0.20.0 · store 0.3.0 · content 0.14.0 ·
foundry 0.9.0, then the variant rename in component 0.131.0.

### The constraint list it set — all four met

- **ONE component, card and row form** → `ContentCard` / `ContentRow`, one
  variant vocabulary across both.
- **Slots, not fields** → `title` · `body` · `meta` · `date` · `tags` ·
  `actions` · `media`, plus `specs` added the same day.
- **Geometry as a prop, not baked** → `ratio`, `thumb`, `paddingY`.
- **Domain behaviour out of the component, on a slot** → the 3D flip and the
  image fade became `flip` / `fade` props; the tilt, the specimen and the
  reveal ride `media` / `footer` / `reveal`.

Point 3 of its findings — that `GridCard` named two unrelated components — was
ruled 2026-08-27 (keep kol-dashboards'), and the shell one is now gone, so the
collision no longer exists.

**Remainder here:** none. Screenshots stay in `_assets/` as the record of what
the old renders looked like.
