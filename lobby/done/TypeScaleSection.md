...
---
## ✅ RESOLVED 2026-07-30 — recipe documented, not built
Per its own recreation note: PageSection + Table exist, so this is a columns recipe. Documented live at `showcase/src/docs/components/Table.mdx` → "Recipes → Type-scale table" (`typeScaleColumns(family, sample)` — token/weight/size-LH/preview columns; cap + pangram are parameters, family defaults to a token).
