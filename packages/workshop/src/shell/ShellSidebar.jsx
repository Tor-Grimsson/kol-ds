  ...
  return (
    <div className="space-y-4">
      <div className="shell-sidebar-toggle shell-sidebar-label" style={{ justifyContent: 'space-between', paddingRight: '4px' }}>
        {labelTo ? (
          <Link to={labelTo} className="shell-sidebar-label kol-helper-14" onClick={(e) => {
          ...
          <button type="button" className="kol-helper-14" onClick={handleToggle}>{label}</button>
        )}
        <button
          type="button"
        ...
        {routes.map((route) => {
          ...
          return (
            <div key={route.id} className="shell-nav-group">
              <button
                type="button"
                className="shell-nav-group-header w-full text-left kol-helper-14"
              ...
              >
                ...
                {route.children?.length > 0 && (
                  <span className="shell-nav-group-count kol-helper-14">({route.children.length})</span>
                )}
              </button>
                  ...
                  {route.children.map((child) => {
                    ...
                    return (
                      <NavLink
                        ...
                        onClick={onNavigate}
                      >
                        <span className="shell-nav-item-title kol-helper-14">{child.label}</span>
                      </NavLink>
                    )
                  })}
                </div>
