...
import { CanvasZoomContext as DsCanvasZoomContext } from '@kolkrabbi/kol-component'
...
function PanZoomViewport({
  ...
  return (
    ...
    {/* The DS chrome (SelectionOverlay) reads the PACKAGE's zoom context, not
        ours — kol-component 0.187.0 restored the screen-constant division and
        keys it off its own context. Publishing the same value into both is a
        two-line bridge; the alternative is swapping this whole viewport for
        the package's PanZoomViewport, which is a structural change, not an
        adoption. Drop this when that swap is deliberately taken. */}
    <DsCanvasZoomContext.Provider value={view.zoom}>
    <div
      ref={containerRef}
    ...
    >
      ...
      <div className="absolute bottom-3 right-3 z-[3] flex items-center gap-2">
        <button
          ...
          className="px-2 py-1 rounded border border-oq-08 bg-surface-secondary kol-mono-12 text-emphasis tabular-nums"
