...
export default {
  ...
  params: [
    ...
    { key: 'rows', label: 'Lines', type: 'range', min: 8, max: 260, step: 1, default: 120, when: isRows, section: 'Geometry' },
    { key: 'rayCount', label: 'Rays', type: 'range', min: 16, max: 520, step: 1, default: 200, when: (l) => l.geometry === 'radial', section: 'Geometry' },
    { key: 'ringCount', label: 'Rings', type: 'range', min: 4, max: 200, step: 1, default: 60, when: (l) => l.geometry === 'rings', section: 'Geometry' },
    { key: 'turns', label: 'Turns', type: 'range', min: 1, max: 30, step: 0.5, default: 6, when: (l) => l.geometry === 'spiral', section: 'Geometry' },
    { key: 'arms', label: 'Arms', type: 'range', min: 1, max: 8, step: 1, default: 1, when: (l) => l.geometry === 'spiral', section: 'Geometry' },
    { key: 'swirl', label: 'Swirl', type: 'range', min: -1, max: 1, step: 0.02, default: 0, when: (l) => l.geometry === 'radial' || l.geometry === 'rings', section: 'Geometry' },
    { key: 'weave', label: 'Weave', type: 'toggle', default: false, when: isRows, section: 'Geometry' },
    /* Spacing — the cumulative-sum gap range + luma contrast. */
    { key: 'minGap', label: 'Min gap', type: 'range', min: 1, max: 24, step: 0.5, default: 3, section: 'Spacing' },
    { key: 'maxGap', label: 'Max gap', type: 'range', min: 4, max: 64, step: 0.5, default: 22, section: 'Spacing' },
    { key: 'contrast', label: 'Contrast', type: 'range', min: 0.3, max: 4, step: 0.05, default: 1.2, section: 'Spacing' },
    ...
    { key: 'markSize', label: 'Mark size', type: 'range', min: 0.2, max: 3, step: 0.05, default: 1, when: (l) => (l.mark ?? 'dots') !== 'glyph', section: 'Mark' },
    { key: 'dashLen', label: 'Dash length', type: 'range', min: 0.3, max: 3, step: 0.05, default: 1.2, when: (l) => l.mark === 'dash', section: 'Mark' },
    { key: 'charset', label: 'Charset', type: 'select', options: CHARSET_OPTIONS, default: 'ascii', when: (l) => l.mark === 'glyph', section: 'Mark' },
    { key: 'fontScale', label: 'Font size', type: 'range', min: 0.4, max: 2, step: 0.05, default: 1, when: (l) => l.mark === 'glyph', section: 'Mark' },
    /* Color. */
