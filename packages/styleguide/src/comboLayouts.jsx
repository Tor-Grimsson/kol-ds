/**
 * ComboLab — stage layout primitives.
 *
 * Each layout takes `{ palette, logo }` and renders its geometry with the
 * palette's role-colors (primary / secondary / light / dark / accent).
 * Structural CSS is DS-owned under `.kol-combo-*`
 * (packages/theme/kol-components-styleguide.css → currently shipped from
 * packages/framework/kol-framework.css). Inline styles here are strictly
 * data-driven (background + fg-on color from the palette) — the classes carry
 * everything else, so nothing is re-invented.
 *
 * DECOUPLING NOTE: the monorepo original imported a hard `KolLogo` brand
 * component. Here `logo` is a consumer-injected React node — <LogoSlot/>
 * renders whatever the host passes (a logomark, an <img>, an <svg>) and
 * renders nothing when it's absent. No brand-app coupling.
 */
import { fgOn } from './comboMath.js'

/* LogoSlot — renders the injected logo node at a fixed inline width. */
...
export function StripeRow({ palette }) {
  ...
  /* Rule color = dark at ~12% alpha, passed via CSS custom prop so the
     .kol-combo-stripe-neutral class owns the border declarations. */
...
/* ────────── Applied card — brand-application "business card" mockup ────────── */
export function AppliedCard({ palette, logo }) {
  ...
  return (
    <div className="kol-combo-stage kol-combo-frame kol-combo-stage--applied" style={{ background: light }}>
      {/* Main logo plate — the card front */}
      ...
      {/* Right swatch column — surface + accent chips + bands */}
...
/* Layout metadata — id + label for pickers. Kept beside the registry so the
   two can't drift. */
export const COMBO_LAYOUTS = [
